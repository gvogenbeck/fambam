const { redis, getJSON, setJSON, countsKey, votersKey, readJsonBody, blockIfArchived } = require('../lib/store');
...
module.exports = async function handler(req, res){
  ...
  try{
    ...
    /* a poll removed entirely — drop its vote hashes */
    ...
    await Promise.all(removedPolls.map(function(id){
      return Promise.all([ redis.del(countsKey(id)), redis.del(votersKey(id)) ]);
    }));

    /* an option removed from a poll that still exists — drop just that field */
